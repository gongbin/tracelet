...
function shapes(sym: SymbolDef, color: string, ghost: boolean) {
  ...
  return <g stroke={color} strokeLinecap="round" strokeLinejoin="round" fill="none">
    {(sym.shapes ?? []).map((sh, i) => {
      ...
      return <text key={i} x={sh.x} y={sh.y} fontSize={sh.size} fill={color} stroke="none" textAnchor="middle">{sh.text}</text>;
    })}
  </g>;
}
...
function body(sym: SymbolDef, color: string, ghost: boolean) {
  ...
  switch (sym.graphic) {
    ...
    case 'resistor':
      return <path d={resistorZigzag(w, h).map((p, i) => `${i ? 'L' : 'M'}${p.x} ${p.y}`).join('')} stroke={color} strokeWidth={SW} fill="none" strokeLinejoin="round" />;
    case 'capacitor':
    ...
    case 'gnd':
      return <path d={GND_BARS.map((f, k) => `M${(w * (1 - f)) / 2} ${k * 50}H${(w * (1 + f)) / 2}`).join('')} stroke={color} strokeWidth={SW} fill="none" />;
    case 'power':
      return <g stroke={color} strokeWidth={SW} fill="none"><circle cx={w / 2} cy={40} r={40} fill={fill} /><path d={`M${w / 2} 80V${h}`} /></g>;
  }
}
...
export function SymbolGlyph({ comp, sym, selected, ghost, wireMode, openPins, pinNets, highlightNet, onBodyDown, onPinDown }: GlyphProps) {
  ...
  void big;
  const { ref: refPos, value: valPos } = symbolTextPositions(comp, sym);
