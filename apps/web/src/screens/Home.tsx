      ...
      <div className="home-main">
        ...
        <div className="action-cards">
          ...
          <div className="action-card" onClick={() => fileRef.current?.click()}>
            ...
            <div className="small muted">KiCad：多选全部 .kicad_sch + .kicad_pcb 或拖整个工程 zip · Altium：多选 .SchDoc + .PcbDoc · 嘉立创 EDA 标准版：文件 → 导出 → EasyEDA 源码（JSON）· 也支持 .eda.json / 备份 zip</div>
            <input ref={fileRef} type="file" accept=".SchDoc,.PcbDoc,.json,.eda.json,.zip,.kicad_sch,.kicad_pcb,.kicad_pro,.eprj,.esch,.epcb" multiple hidden onChange={(e) => { const fs = e.target.files ? Array.from(e.target.files) : []; if (fs.length) void onImport(fs); e.target.value = ''; }} />
          </div>
