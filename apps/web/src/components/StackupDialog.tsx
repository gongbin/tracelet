...
export function StackupDialog({ close }: { close: () => void }) {
  ...
  const dielectric = b.copperCount === 4 ? [(b.thickness - 4 * cuUm(st.copperWeight) / 1000) * 0.2, (b.thickness - 4 * cuUm(st.copperWeight) / 1000) * 0.6, (b.thickness - 4 * cuUm(st.copperWeight) / 1000) * 0.2] : [b.thickness - 2 * cuUm(st.copperWeight) / 1000];
  ...
  cu.forEach((l, i) => {
    ...
    if (i < cu.length - 1) rows.push({ name: b.copperCount === 4 && i === 1 ? '芯板 FR-4' : b.copperCount === 4 ? '半固化片' : '芯板 FR-4', kind: b.copperCount === 4 && i !== 1 ? 'pp' : 'core', t: `${dielectric[i].toFixed(2)} mm` });
  });
  return (
    <div className="overlay" onClick={close}>
      <div className="dialog" style={{ width: 620 }} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
        ...
        <div className="dialog-body row" style={{ gap: 20, alignItems: 'flex-start' }}>
          <div className="kv grow" style={{ gap: '10px 12px' }}>
            <span className="k">层数</span><div className="seg" style={{ width: 200 }}><span className={`seg-opt${b.copperCount === 2 ? ' on' : ''}`} onClick={() => { if (b.copperCount !== 2) { if (app.activeLayer.startsWith('In')) app.set('activeLayer', 'F.Cu'); editor.dispatch(pcb.setCopperCount(2)); } }}>2 层</span><span className={`seg-opt${b.copperCount === 4 ? ' on' : ''}`} onClick={() => b.copperCount !== 4 && editor.dispatch(pcb.setCopperCount(4))}>4 层</span></div>
            ...
            {b.copperCount === 4 && <><span className="k">内层铜厚</span><div className="row" style={{ gap: 4 }}>{[0.5, 1].map((oz) => <span key={oz} className={`chip mono${st.innerCopperWeight === oz ? ' on' : ''}`} onClick={() => set({ innerCopperWeight: oz })}>{oz} oz</span>)}</div></>}
