...
export function Panelization(){
  ...
  useEffect(()=>{if(app.fabPanelOpen)details.current?.scrollIntoView?.({block:'nearest'});},[app.fabPanelOpen]);
  const [options,setOptions]=useState<PanelOptions>({columns:2,rows:1,gap:5,rail:5,tabWidth:3,mouseBiteDrill:.5,mouseBitePitch:.85});
  ...
  return <details ref={details} open={app.fabPanelOpen} onToggle={e=>{const open=e.currentTarget.open;if(open!==app.fabPanelOpen)app.set('fabPanelOpen',open);}}><summary>{zh?'拼板 · 独立制造输出':'Panelization · separate fabrication output'}</summary><div className="col" style={{gap:12,paddingTop:12}}>
  <div className="row" style={{flexWrap:'wrap'}}>{(Object.keys(options) as (keyof PanelOptions)[]).map((k,i)=><label key={k}>{zh?['列','行','间隔 mm','工艺边 mm','连接桥 mm','鼠咬孔径 mm','鼠咬孔中心距 mm'][i]:k}<input className="input" type="number" style={{width:75}} value={options[k]} onChange={e=>{setOptions({...options,[k]:e.target.valueAsNumber});setFiles(null);}}/></label>)}</div>
  ...
  {plan && <svg viewBox={`-2 -2 ${plan.width+4} ${plan.height+4}`} style={{width:'100%',maxHeight:300,background:'#151c22'}} aria-label="Panel preview">{plan.rails.map((r,i)=><rect key={`r${i}`} {...r} width={r.w} height={r.h} fill="#516454"/>)}{plan.instances.map(i=><polygon key={i.id} points={i.outline.map(p=>`${p.x},${p.y}`).join(' ')} fill="#184b31" stroke="#b8ce85" strokeWidth={0.2}/>)}{plan.tabs.map((r,i)=><rect key={`t${i}`} x={r.x} y={r.y} width={r.w} height={r.h} fill="#daa447"/>)}{plan.holes.map((p,i)=><circle key={`hole${i}`} cx={p.x} cy={p.y} r={(plan.options.mouseBiteDrill??.5)/2} fill="#101820" stroke="#d0d2d6" strokeWidth={.04}/>)}{plan.fiducials.map((p,i)=><circle key={`f${i}`} {...{cx:p.x,cy:p.y}} r={0.5} fill="#eac36f"/>)}</svg>}
