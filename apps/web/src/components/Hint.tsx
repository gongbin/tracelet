...
export function Hint({ space }: { space: 'sch' | 'pcb' }) {
  ...
  if (space === 'sch') {
    ...
    else { title = '选择'; body = '拖动移动 · 点引脚开始连线 · A 放元件 · R 旋转 · Del 删除 · 空格+拖动平移'; }
  } else {
    const m: Record<string, [string, string]> = {
      select: ['选择', '点焊盘高亮网络 · 拖动元件 · R 旋转 · F 翻面 · Del 删除走线'],
      ...
      edge: ['板框', '在制造页设置尺寸'], text: ['丝印文字', '点击放置'], measure: ['测量', '点两点 · 再点重新开始 · Esc 结束'],
      flip: ['翻面', '点击元件翻到另一面'], align: ['对齐 / 分布', '先框选多个元件'], autoroute: ['自动布线', '下一里程碑接入 Freerouting'], refill: ['重填铺铜', '铺铜实时计算']
    };
