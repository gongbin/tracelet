...
import { approximateCatalogKey, catalogModel, modelFor, MODEL_CATALOG } from '../src/editors/three/models.js';
import { registerProjectLibrary, type Board, type BoardFootprint } from '@tracelet/kernel';

describe('3D 模型匹配', () => {
  it('USB-C 座没有精确模型时仅提供同系列候选，不自动替换（HRO → GCT_USB4085）', () => {
    ...
    const candidate = catalogModel(f, 'USB_C_Receptacle_GCT_USB4085');
    expect(candidate.source).toBe('catalog:USB_C_Receptacle_GCT_USB4085');
    expect(candidate.provenance?.kind).toBe('approximate');
    expect(modelFor(f, { models3d: { [f.footprintId]: candidate } } as unknown as Board)).toEqual(candidate);
  });
  it('近似模型按焊盘区中心对齐（HRO 焊盘中心 (0,-1.46) vs GCT4085 (2.975,2.43)）', () => {
    ...
    const f = { id: 'f', ref: 'J9', footprintId: 'fp:kicad:USB_C_Receptacle_HRO_TYPE-C-31-M-12', x: 0, y: 0, rotation: 0, side: 'F', padNets: {} } as BoardFootprint;
    const m = catalogModel(f, 'USB_C_Receptacle_GCT_USB4085');
