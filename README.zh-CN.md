...
- **轻量级 TypeScript EDA**：应用与 EDA 内核使用 TypeScript 开发，构建后即可静态托管，在浏览器中运行；日常本地设计无需安装桌面客户端、数据库或原生 EDA 内核。
...
Tracelet 将设计工作区交付为一组静态网页资源。打开网页即可开始本地设计，无需先下载 GB 级桌面安装包或整套离线器件库。应用与 EDA 内核采用 TypeScript，原理图、布局、布线和检查逻辑在浏览器中执行；自动布线使用 Web Worker。

以 `57b17a6` 版本的生产构建为例，`apps/web/dist` 全部文件合计约 **25.4 MiB**，包含随应用分发的 3D 模型及图标等资源。这是未压缩静态产物的体积，不是首屏下载量或运行内存，也不包含开发依赖、项目数据和可选服务端；资源库扩充后体积会变化。

```bash
pnpm install
pnpm build
pnpm --filter @tracelet/web preview  # 本地预览生产构建
```

发布网页只需将 `apps/web/dist` 放到支持 SPA 路由回退的静态托管服务上，将项目页面路径回退到 `index.html`；不需要在运行网页的服务器上安装 Node.js 或数据库。构建环境需要 Node.js 与 pnpm。请通过 HTTP(S) 访问，不能仅双击 `index.html` 使用。

本地设计与制造导出不要求启动后端。远程共享存储、AI 服务和在线报价按需启用，分别需要相应服务或网络连接；3D 模型转换等辅助工具不属于日常浏览器运行的必需项。
...
**装配页**提供分组物料清单、板面与旋转视图、元件高亮和完成标记。**测试页**根据网络和元件生成板上探测位置及检查建议，不控制仪器，也不认证电路功能。**3D 页**提供独立产品渲染：顶面 / 底面 / 等轴视角、浅色 / 深色 / 透明背景，最高 2400 × 1800 PNG；未匹配模型仍明确显示为占位。模型匹配提供外部搜索、本地 STEP / STP → GLB 转换、焊盘叠加预览和可撤销校准，来源信息与文件指纹随项目保存。STEP 上限 20 MB，嵌入 GLB 上限 8 MB，转换网格上限 20 万三角面。OCCT WebAssembly 转换器在首次导入 STEP 时加载（未压缩约 7.6 MB），转换不上传文件。
...
- 已支持 GLB 和 PNG。可在模型匹配中导入 STEP / STP 并在浏览器本地转换；暂不提供原生 STEP 导出或多人实时共同编辑。
...
- MCU 零件，模块与裸片两种形态都有：ESP32-S3-WROOM-1 / S3-MINI-1 / S31-WROOM-3 / C3-MINI-1 / C3-WROOM-02 / C6-WROOM-1 / C5-WROOM-1 模块（官方 castellated 封装）；ESP32-S3 / C3 / C5 / C6 / H2 SoC 裸片（乐鑫推荐的 QFN 焊盘图）；STM32F103C8T6 / CBT6、STM32F030F4P6、STM32G030F6P6、STM32F411CEU6、STM32L432KCU6（参数化生成的 LQFP / TSSOP / UFQFPN 封装）。全部带完整引脚表，由 `scripts/build-mcu-lib.mjs` 从乐鑫官方库与 KiCad 官方库解析生成，不是手抄
...
- 3D：Three.js 真实几何（板体带孔、焊盘、走线、铺铜、丝印）+ KiCad 标准库 3D 模型（244 个常用封装转成 GLB，按封装名称精确自动匹配，近似候选需人工确认；可搜索外部模型网站、本地导入 STEP / STP 或 GLB，叠加焊盘预览并校准缩放、旋转、偏移后保存），截图 PNG / 导出 GLB，点选联动 PCB 选择
...
[GPL-3.0-or-later](./LICENSE)，与 KiCad 相同。允许使用、修改与商业销售；分发修改版时必须以同一协议公开源码。内核、CLI、Web 与服务端均适用。第三方资源沿用各自协议（KiCad 3D 模型：CC-BY-SA 4.0 附 KiCad 库例外条款，见 `apps/web/public/models3d`）。STEP 转换使用 occt-import-js / Open CASCADE Technology，许可和源码声明见 `apps/web/public/licenses`。
