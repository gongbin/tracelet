...
- PCB：框选、板框编辑、走线顶点/线段拖动、走线中实时间距检查、对齐分布、内置自动布线（全局最短优先严格布线 + 失败网络优先的拆线重布、真实铜边距冲突图、45° 倒角、层方向偏好；Web Worker 运行，进度 / 取消，建议预览后接受）、开孔工具（M2–M4 螺丝孔 / 金属化孔）、仅板级封装（定位孔 / 基准点 / Logo）直接放置、板框改尺寸 / 整板拖动 / 适配内容、状态栏栅格 / 线宽 / 过孔选择、层叠与工艺参数（板厚、铜厚、表面处理、阻焊 / 丝印颜色）
