import { powerThermalReport } from './powerThermal.js';
...
const SIGNAL=new Set(['allowed-layers','max-length','neckdown','reference-plane','differential-gap','differential-skew','differential-incomplete','differential-unverified','parallel-exposure','return-via','return-via-unassessed']);
export function engineeringReport(project:Project, cached?:{drc:ReturnType<typeof runDrc>;ratsnest:ReturnType<typeof computeRatsnest>;erc?:ReturnType<typeof runSchematicErc>}){
 ...
 const signalConfigured=b.netClasses.filter(n=>n.maxLength!==undefined||n.allowedLayers||n.referenceLayer||n.referenceNet||n.neckdown||n.engineering).length+(b.differentialPairs?.length??0);
 ...
 const shorts=copperShorts(b,rules);
 const routing:EngineeringIssue[]=issues(shorts,{error:15,warning:0,info:0});
 ...
 if(!populated)blockers.push('empty');if(b.outline.length<3)blockers.push('outline');if(rats.unrouted)blockers.push('unrouted');if(drc.errors)blockers.push('drc');if(erc.report?.errors)blockers.push('erc');if(shorts.length)blockers.push('copper-short');
 ...
 return {version:1,projectId:project.id,projectName:project.name,updatedAt:project.updatedAt,ruleSet:rules.name,score:weight?Math.round(assessed.reduce((s,c)=>s+c.score!*c.weight,0)/weight):null,assessed:assessed.length,categories,blockers,erc,
  ...
  nets:stats.nets,impedance,power:powerThermalReport(b),
