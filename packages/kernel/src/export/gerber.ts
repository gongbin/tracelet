...
export interface FabFile { name: string; content: string; kind: 'gerber' | 'drill' | 'bom' | 'pnp' | 'netlist' | 'project' | 'readme' }

const slugOf = (name: string) => name.replace(/[^\w一-龥-]+/g, '-').replace(/^-|-$/g, '') || 'board';

/** 全套制造文件（嘉立创/JLCPCB 可识别的 Protel 扩展名）。 */
export function exportFabFiles(project: Project, opts: { bom?: boolean; pnp?: boolean; netlist?: boolean; project?: boolean } = {}): FabFile[] {
  ...
  files.push({ name: 'README.txt', kind: 'readme', content: [`${project.name}`, `Generated by Tracelet ${VERSION} · ${new Date().toISOString()}`, `Layers: ${board.copperCount} · Thickness: ${board.thickness}mm · Rules: ${rules.name}`, '', 'Gerber RS-274X (4.6 mm), Excellon metric decimal.', 'Solder mask files are negative (openings). Vias are tented.', ''].join('\n') });
  return files;
}
